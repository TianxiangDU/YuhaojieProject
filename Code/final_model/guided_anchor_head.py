...
from mmdet.ops import DeformConv, MaskedConv2d, ModulatedDeformConv
...
class FeatureAdaption(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_mask = nn.Conv2d(2, deformable_groups * mask_channels, 1, bias=False) 
    ...
    def init_weights(self):
        ...
        normal_init(self.conv_mask, std=0.1)
        normal_init(self.conv_adaption, std=0.01)

    def forward(self, x, shape):
        offset = self.conv_offset(shape.detach())
        mask = self.conv_mask(shape.detach())
        offset = torch.zeros_like(offset, requires_grad=False)
